...
export const signup = async (req, res) => {
  try {
    ...
    if (existingUser) {
      return res.status(409).json({ success: false, msg: "Email or username already exists" });
    }
    ...
    logError(error);
    res.status(500).json({ success: false, msg: "Unable to sign up, try again later" });
  }
};
...
export const login = async (req, res) => {
  try {
    ...
    logError(error);
    res.status(500).json({ success: false, msg: "Unable to login, try again later" });
  }
};
...
export const updateProfile = async (req, res) => {
  try {
    const updates = {};
    if (typeof req.body.bio === "string") updates.bio = req.body.bio.slice(0, 300);
